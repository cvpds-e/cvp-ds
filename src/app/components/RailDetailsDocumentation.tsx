...
export function RailDetailsDocumentation() {
  ...
  return <main className="rail-details-docs documentation-container">
    ...
    <section className="rail-details-docs__section" aria-labelledby="rail-details-composition"><div className="rail-details-docs__section-heading"><span>Integration composition</span><h2 id="rail-details-composition">Built from the system</h2></div><div className="rail-details-docs__grid"><article><PanelLeft size={20} /><strong>Configuration pane</strong><p>Tabs, Text Input, Number Input, Select, Checkbox, Search Field, and Sort Control share one compact form rhythm.</p></article><article><Rows3 size={20} /><strong>Content Query</strong><p>Tag Filter and Multi Select support program type, tags, media availability, release year, and subscription-package criteria; each contextual label uses the shared Tooltip pattern.</p></article><article><ListFilter size={20} /><strong>Empty rail choices</strong><p>The elevated empty state routes algorithmic curation to Content Query and manual curation to Content Browser Modal.</p></article></div></section>
