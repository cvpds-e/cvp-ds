...
const selectTokenRows: ComponentTokenContractRow[] = [
  ...
  { role:'Modal popup layer', token:'--cvp-select-popup-modal-z', source:'--cvp-z-modal + 1', contract:'Portaled listbox remains above its dialog', activation:'Select inside Modal' },
...
export function SelectDocumentation(){
  ...
  return <main className="cvp-input-doc">
    ...
    <section className="cvp-input-doc__grid"><article className="cvp-input-doc__panel"><header><b>05</b><div><h2>Keyboard contract</h2></div></header><ul><li><code>Enter</code> or <code>Space</code> opens and selects.</li><li>Arrow keys move through enabled options.</li><li><code>Home</code> and <code>End</code> move to boundaries.</li><li><code>Escape</code> closes without changing value.</li><li><code>Tab</code> closes and continues naturally.</li></ul></article><article className="cvp-input-doc__panel"><header><b>06</b><div><h2>Accessibility contract</h2></div></header><ul><li>Combobox, listbox, and option roles form one named control.</li><li><code>aria-activedescendant</code> exposes virtual focus.</li><li>Selected and disabled options expose explicit states.</li><li>The portaled listbox uses the modal overlay layer when its trigger is inside a Modal.</li><li>A hidden named value participates in form submission.</li><li>Use native Select when custom presentation is unnecessary.</li></ul></article></section>
