...
export function TextareaDocumentation() {
  ...
  return <main className="cvp-input-doc">
    <header className="cvp-input-doc__hero"><div><span>Component 02 · Standardized</span><h1>Textarea</h1></div><div><p>Multi-line input that inherits the CVP field contract while protecting long-form entry, resizing, and character limits.</p><span className="cvp-input-doc__status">Ready for implementation</span></div></header>
    ...
    <section className="cvp-input-doc__panel"><header><b>02</b><div><h2>Complete state model</h2><p>Textarea shares field states with Text Input.</p></div></header><div className="cvp-input-doc__states"><Textarea label="Default" placeholder="Enter supporting context"/><Textarea label="Hover" data-preview-state="hover" defaultValue="Pointer state"/><Textarea label="Focus" data-preview-state="focus" defaultValue="Keyboard focus"/><Textarea label="Read only" readOnly value="Approved copy cannot be edited."/><Textarea label="Required" required helperText="Add a reason before continuing."/><Textarea label="Invalid" defaultValue="Too short" error="Provide at least 20 characters of useful context."/><Textarea label="Disabled" disabled value="Unavailable content"/><Textarea label="Fixed resize" resize="none" defaultValue="Resize is intentionally disabled in constrained layouts."/></div></section>
    ...
    <section className="cvp-input-doc__panel"><header><b>04</b><div><h2>Token contract</h2><p>Textarea extends the shared field contract without bypassing semantic roles.</p></div></header><ComponentTokenContract label="Textarea token contract" rows={textAreaTokenRows}/></section>
