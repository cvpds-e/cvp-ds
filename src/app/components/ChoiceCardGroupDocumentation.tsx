...
const tokenRows: ComponentTokenContractRow[] = [
  ...
  { role: 'Selected surface', token: '--cvp-choice-card-bg-selected', source: '--cvp-color-surface-subtle', contract: 'Quiet persistent selected cue', activation: 'aria-checked=true' },
...
export function ChoiceCardGroupDocumentation() {
  ...
  return <main className="cvp-input-doc">
    ...
    <section className="cvp-input-doc__panel"><header><b>01</b><div><h2>Anatomy and live behavior</h2><p>A compact radio indicator, quiet selected surface, and concise supporting copy keep comparisons scannable.</p></div></header><div className="cvp-input-doc__live"><ChoiceCardGroup label="Rail type" helperText="Choose how content is selected." required columns={3} options={options} value={value} onChange={setValue} /></div></section>
    ...
    <section className="cvp-input-doc__grid"><article className="cvp-input-doc__panel"><header><b>04</b><div><h2>Accessibility contract</h2></div></header><ul><li>The container exposes a named <code>radiogroup</code>.</li><li>Helper text is associated with the group through <code>aria-describedby</code>; required groups expose <code>aria-required</code>.</li><li>Every card exposes its selected state with <code>aria-checked</code> and a persistent radio indicator.</li><li>Arrow keys move and select; Home and End jump to boundaries.</li><li>Only one enabled option participates in the tab order.</li></ul></article><article className="cvp-input-doc__panel"><header><b>05</b><div><h2>Usage decisions</h2></div></header><ul><li>Use when descriptions or badges help users compare choices.</li><li>Use SegmentedControl for compact labels and immediate view changes.</li><li>Keep choices mutually exclusive and descriptions parallel.</li><li>Prefer two or three columns; the component reflows on narrow screens.</li></ul></article></section>
  </main>;
}