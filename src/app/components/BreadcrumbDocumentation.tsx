...
import './BreadcrumbDocumentation.css';
...
export function BreadcrumbDocumentation() {
  ...
  return <main className="cvp-input-doc cvp-breadcrumb-doc">
    <header className="cvp-input-doc__hero"><div><span>Navigation 01 · Standardized</span><h1>Breadcrumb</h1></div><div><p>A compact location trail for hierarchical CVP workflows, with predictable ancestor navigation and optional related-destination menus.</p><span className="cvp-input-doc__status">Ready for implementation</span></div></header>

    <section className="cvp-input-doc__panel"><header><b>01</b><div><h2>Anatomy and live behavior</h2><p>Ancestor, separator, and current-page roles remain visually and semantically distinct.</p></div></header><div className="cvp-input-doc__anatomy"><div className="cvp-input-doc__live"><Breadcrumb items={dropdownItems}/><p className="cvp-breadcrumb-doc__result">Selected destination: <strong>{destination}</strong></p></div><ol><li><b>1</b><span><strong>Back ancestor</strong> Returns to the parent collection.</span></li><li><b>2</b><span><strong>Separator</strong> Communicates hierarchy without receiving focus.</span></li><li><b>3</b><span><strong>Ancestor menu</strong> Offers sibling destinations when the hierarchy warrants it.</span></li><li><b>4</b><span><strong>Current page</strong> Uses <code>aria-current="page"</code> and is not interactive.</span></li></ol></div></section>

    <section className="cvp-input-doc__panel"><header><b>02</b><div><h2>Variants and states</h2><p>Examples cover the supported hierarchy patterns without inventing parallel styling.</p></div></header><div className="cvp-breadcrumb-doc__states"><article><span>Two levels</span><Breadcrumb items={basicItems}/></article><article><span>Rail Details canvas</span><Breadcrumb surface="canvas" items={[{id:'rails-list',label:'Rails List',href:'#rails-list'},{id:'trending',label:'Trending'}]}/></article><article><span>Long hierarchy</span><Breadcrumb items={[{id:'home',label:'Home',icon:Home,href:'#home'},{id:'library',label:'Content library',icon:Folder,href:'#library'},{id:'series',label:'Series',icon:Play,href:'#series'},{id:'episode',label:'A deliberately long episode title that truncates safely'}]}/></article><article><span>Custom separator</span><Breadcrumb separator={<span>/</span>} items={[{id:'docs',label:'Documents',href:'#docs'},{id:'reports',label:'Reports',href:'#reports'},{id:'q3',label:'Q3 report'}]}/></article><article><span>Single location</span><Breadcrumb items={[{id:'dashboard',label:'Dashboard',icon:Home}]}/></article></div></section>

    <section className="cvp-input-doc__panel"><header><b>03</b><div><h2>Theme parity</h2><p>Hierarchy and menu layers resolve from the same semantic contract in both themes.</p></div></header><div className="cvp-input-doc__themes"><article data-theme="dark"><span>Dark</span><Breadcrumb items={basicItems}/></article><article data-theme="light"><span>Light</span><Breadcrumb items={basicItems}/></article></div></section>

    <section className="cvp-input-doc__panel"><header><b>04</b><div><h2>Token contract</h2><p>Breadcrumb owns a registered Tier 3 contract and composes the shared menu foundation for dropdowns.</p></div></header><ComponentTokenContract label="Breadcrumb token contract" rows={tokenRows}/></section>
