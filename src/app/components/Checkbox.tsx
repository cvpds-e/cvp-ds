...
export interface CheckboxProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>,'checked'|'defaultChecked'|'onChange'|'type'>{checked?:CheckboxState;defaultChecked?:CheckboxState;onChange?:(checked:CheckboxState)=>void;label?:string;description?:string;error?:string;invalid?:boolean;previewState?:'hover'|'focus';}

export const Checkbox=forwardRef<HTMLInputElement,CheckboxProps>(({id,checked,defaultChecked=false,onChange,disabled=false,label,description,error,invalid=false,previewState,className='',required=false,'aria-describedby':ariaDescribedBy,'aria-invalid':ariaInvalid,...props},forwardedRef)=>{
 ...
 const setRef=(node:HTMLInputElement|null)=>{localRef.current=node;if(typeof forwardedRef==='function')forwardedRef(node);else if(forwardedRef)(forwardedRef as React.MutableRefObject<HTMLInputElement|null>).current=node};
 ...
 return <label className={['cvp-checkbox',className].filter(Boolean).join(' ')} data-state={current==='indeterminate'?'indeterminate':current?'checked':'unchecked'} data-preview-state={previewState} data-disabled={disabled||undefined} data-invalid={hasError||undefined}>
