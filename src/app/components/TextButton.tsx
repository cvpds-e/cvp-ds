...
export function TextButton({children,variant='default',disabled=false,className='',icon,active=false,loading=false,loadingText='Loading',...props}:TextButtonProps){
 ...
 const content=<span className="cvp-text-button__content">{icon&&<span className="cvp-text-button__icon" aria-hidden="true">{icon}</span>}<span>{loading?loadingText:children}</span>{loading&&<Loader size="sm" tone="inherit" decorative/>}</span>;
