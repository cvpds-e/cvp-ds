...
export function Select({ options, value, defaultValue = '', onChange, placeholder = 'Select an option…', disabled = false,
  ...
  return <div ref={rootRef} className={['cvp-select', `cvp-select--${size}`, variant === 'button' && 'cvp-select--button', className].filter(Boolean).join(' ')} data-invalid={Boolean(error) || undefined} data-disabled={disabled || undefined}>
    ...
    {isOpen && typeof document !== 'undefined' && createPortal(<div ref={popupRef} className={`cvp-select__popup ${triggerRef.current?.closest('.cvp-modal') ? 'cvp-select__popup--modal' : ''}`.trim()} style={{ position: 'fixed', top: popupPosition.top, left: popupPosition.left, width: popupPosition.width, maxHeight: popupPosition.maxHeight }}><ul id={listboxId} className="cvp-select__listbox" role="listbox" aria-labelledby={labelId}>
