...
export function TextButton({children,variant='default',disabled=false,className='',icon,active=false,loading=false,loadingText='Loading',...props}:TextButtonProps){
 ...
 const content=<span className="cvp-text-button__content">{icon&&<span className="cvp-text-button__icon" aria-hidden="true">{icon}</span>}<span>{loading?loadingText:children}</span>{loading&&<LoadingSpinner size="sm" tone="inherit" decorative/>}</span>;
 if(isLink){const {href,...linkProps}=props as LinkProps;return <a {...linkProps} href={unavailable?undefined:href} className={classes} aria-disabled={unavailable||undefined} aria-current={active?'page':undefined} data-loading={loading||undefined} tabIndex={unavailable?-1:linkProps.tabIndex}>{content}</a>}
