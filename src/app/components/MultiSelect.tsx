...
export function MultiSelect({options,value,defaultValue=[],onChange,id,name,label,labelTooltip,description,placeholder='Select options…',maxSelection,disabled=false,required=false,error=false,className='',allowCreate=true,onCreateOption,previewState}:MultiSelectProps){
 ...
 const positionPopup=()=>{const rect=shellRef.current?.getBoundingClientRect();if(rect)setPopupPosition({top:rect.bottom+4,left:rect.left,width:rect.width})};
 ...
 useEffect(()=>{if(!open)return;positionPopup();window.addEventListener('resize',positionPopup);window.addEventListener('scroll',positionPopup,true);return()=>{window.removeEventListener('resize',positionPopup);window.removeEventListener('scroll',positionPopup,true)}},[open]);
 ...
 return <div ref={rootRef} className={['cvp-multi-select',className].filter(Boolean).join(' ')} data-disabled={disabled||undefined} data-invalid={hasError||undefined} data-has-values={selectedOptions.length>0||undefined} data-open={(open||previewState==='open')||undefined} data-preview-state={previewState}>
  ...
  {open&&typeof document!=='undefined'&&createPortal(<div ref={popupRef} className="cvp-multi-select__popup" style={{position:'fixed',top:popupPosition.top,left:popupPosition.left,right:'auto',width:popupPosition.width}}>
