...
export interface MultiSelectProps{
 ...
 id?:string;name?:string;label?:string;labelTooltip?:React.ReactNode;description?:string;placeholder?:string;maxSelection?:number;
...
export function MultiSelect({options,value,defaultValue=[],onChange,id,name,label,labelTooltip,description,placeholder='Select options…',maxSelection,disabled=false,required=false,error=false,className='',allowCreate=true,onCreateOption,previewState}:MultiSelectProps){
 ...
 const[internal,setInternal]=useState(defaultValue);const[created,setCreated]=useState<MultiSelectOption[]>([]);const[open,setOpen]=useState(false);const[query,setQuery]=useState('');const rootRef=useRef<HTMLDivElement>(null);const shellRef=useRef<HTMLDivElement>(null);const popupRef=useRef<HTMLDivElement>(null);const inputRef=useRef<HTMLInputElement>(null);const listRef=useRef<HTMLUListElement>(null);const[popupPosition,setPopupPosition]=useState({top:0,left:0,width:0,maxHeight:0});
 ...
 const positionPopup=()=>{const rect=shellRef.current?.getBoundingClientRect();if(!rect)return;const viewportInset=8,gap=4,spaceAbove=Math.max(0,rect.top-viewportInset-gap),spaceBelow=Math.max(0,window.innerHeight-rect.bottom-viewportInset-gap),popupHeight=popupRef.current?.getBoundingClientRect().height||320,openUpward=spaceBelow<Math.min(popupHeight,240)&&spaceAbove>spaceBelow,availableHeight=Math.max(112,openUpward?spaceAbove:spaceBelow),renderedHeight=Math.min(popupHeight,availableHeight);setPopupPosition({top:openUpward?rect.top-gap-renderedHeight:rect.bottom+gap,left:rect.left,width:rect.width,maxHeight:availableHeight})};
 const show=()=>{if(disabled)return;positionPopup();setOpen(true);requestAnimationFrame(()=>inputRef.current?.focus())};
 ...
 useEffect(()=>{const outside=(event:MouseEvent)=>{if(!rootRef.current?.contains(event.target as Node)&&!popupRef.current?.contains(event.target as Node))close()};document.addEventListener('mousedown',outside);return()=>document.removeEventListener('mousedown',outside)},[]);
 useEffect(()=>{if(!open)return;positionPopup();const frame=window.requestAnimationFrame(positionPopup);window.addEventListener('resize',positionPopup);window.addEventListener('scroll',positionPopup,true);return()=>{window.cancelAnimationFrame(frame);window.removeEventListener('resize',positionPopup);window.removeEventListener('scroll',positionPopup,true)}},[open]);
 const hasError=Boolean(error),message=typeof error==='string'?error:undefined;
 return <div ref={rootRef} className={['cvp-multi-select',className].filter(Boolean).join(' ')} data-disabled={disabled||undefined} data-invalid={hasError||undefined} data-has-values={selectedOptions.length>0||undefined} data-open={(open||previewState==='open')||undefined} data-preview-state={previewState}>
  ...
  <div ref={shellRef} className="cvp-multi-select__shell" onClick={show}>
   <div className="cvp-multi-select__content">
    {selectedOptions.map(option=><Pills className="cvp-multi-select__tag" key={option.value} onRemove={disabled?undefined:()=>remove(option.value)} removeLabel={`Remove ${option.label}`}>{option.label}</Pills>)}
  ...
  {open&&typeof document!=='undefined'&&createPortal(<div ref={popupRef} className="cvp-multi-select__popup" style={{position:'fixed',top:popupPosition.top,left:popupPosition.left,right:'auto',width:popupPosition.width,maxHeight:popupPosition.maxHeight}}>
